...
class EyetrackingDataPreprocessor(Dataset):
    ...
    def __init__(
        ...
        if {'sex', 'Grade'}.issubset(data.columns) and drop_demographics == True:
            data = data.drop(columns = ['sex', 'Grade'])    
            convert_columns = ['direction']
        elif {'sex', 'Grade'}.issubset(data.columns) and drop_demographics == False:
        ...
        if {'Reading_speed'}.issubset(data.columns) and drop_demographics == False:
        ...
        if {'group'}.issubset(data.columns):
...
class EyetrackingDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        if self.batch_subjects:
            ...
            X = torch.stack([torch.FloatTensor(X) for X, _, _, _ in subject_sentences])
